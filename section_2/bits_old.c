...
main()
{
    ...
    printf("to set on-\n%s\n", int2bin(getbits(2, 1, 2) << 3));
    printf("to set on-\n%s\n", int2bin(getbits(2, 2, 2) << 3));
    printf("set on-\n%s\n", int2bin((getbits(2, 1, 2) << 3) |  (n & ~(~0 << (3+2) ^ ~0 << 3))));
    printf("with func-\n%s\n", int2bin(setbits(n, 3, 2, 2)));
    printf("with func-\n%s\n", int2bin(setbits(-n, 3, 2, 2)));
    printf("original-\n%s\n", int2bin(-n));
    printf("original-\n%s\n", int2bin(n));
...
{
    return (getbits(y, n-1, n) << p) |  (x & ~(~0 << (p+n) ^ ~0 << p));
}
